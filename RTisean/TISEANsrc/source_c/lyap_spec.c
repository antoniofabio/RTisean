...
void make_dynamics(double **dynamics,long act)
{
  ...
  for (d=0;d<DIMENSION;d++) {
    ...
    for (i=0;i<alldim;i++)
      new += dynamics[d][i]*series[indexes[i][1]][t-indexes[i][0]];
    averr[d] += (new-series[d][t+DELAY])*(new-series[d][t+DELAY]);
  }
...
int main(int argc,char **argv)
{
  ...
  check_alloc(vec=(double*)malloc(sizeof(double)*alldim));
  check_alloc(mat=(double**)malloc(sizeof(double*)*alldim));
  for (i=0;i<=alldim;i++)
    check_alloc(mat[i]=(double*)malloc(sizeof(double)*alldim));
