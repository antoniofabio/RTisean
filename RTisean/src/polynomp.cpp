...
void ns_polynomp::show_options(char *progname)
{
...
void ns_polynomp::scan_options(int n,char **in)
{
  char *out;
...
double ns_polynomp::polynom(unsigned long act,unsigned int which)
{
  unsigned int i,j;
  double ret=1.0,h;
...
void ns_polynomp::make_fit(void)
{
  double **mat,*vec;
  double h;
  unsigned long n,hn;
...
double ns_polynomp::forecast_error(unsigned long i0,unsigned long i1)
{
  unsigned int i;
  unsigned long n;
  double h,error=0.0;
...
void ns_polynomp::make_cast(FILE *fcast)
{
  int i,j,hi;
  unsigned int k;
  double casted;
...
int ns_polynomp::main(int argc,char **argv)
{

outfile=NULL;stdo=1;
parin=NULL,infile=NULL;
length=ULONG_MAX;insample=ULONG_MAX;exclude=0;
plength=UINT_MAX;
step=1000;
column=1;dim=2;delay=1;down_to=1;
verbosity=0xff;
