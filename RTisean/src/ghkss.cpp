...
void ns_ghkss::show_options(char *progname)
{
...
void ns_ghkss::scan_options(int n,char **in)
{
  char *out;
...
void ns_ghkss::sort(double *x,int *n)
{
  int i,j,iswap;
  double dswap;
...
void ns_ghkss::make_correction(unsigned long n,unsigned long nf)
{
  int i,i1,j,j1,k,hs;
  double help;
...
void ns_ghkss::handle_trend(unsigned long n,unsigned long nf)
{
  int i,j;
  double *av,help;
...
void ns_ghkss::set_correction(void)
{
  int i;
...
int ns_ghkss::main(int argc,char **argv)
{

length=ULONG_MAX;exclude=0;
dim=5;qdim=3;delay=1;column=1;minn=30;iterations=1;
verbosity=0xff;
eps_set=0;euclidean=0;
outfile=NULL;stdo=1;
infile=NULL;
