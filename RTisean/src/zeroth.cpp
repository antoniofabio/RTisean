/*Author: Rainer Hegger. Last modified: Nov 18, 2000 */
...
void ns_zeroth::show_options(char *progname)
{
...
void ns_zeroth::scan_options(int n,char **in)
{
  char *out;
...
void ns_zeroth::make_fit(long act,unsigned long number,long istep,double **error)
{
  double casted,*help;
  long i,j,h;
...
int ns_zeroth::main(int argc,char **argv)
{
nmax=(NMAX-1);

epsset=0;dimset=0;clengthset=0;causalset=0;
infile=NULL;
outfile=NULL;stdo=1;
COLUMNS=NULL;
embed=2;dim=1;DELAY=1;MINN=30;
STEP=1;causal;
verbosity=0x1;
EPS0=1.e-3;EPSF=1.2;
refstep=1;
LENGTH=ULONG_MAX;exclude=0;CLENGTH=ULONG_MAX;
  ...
  unsigned long actfound,hactfound;
