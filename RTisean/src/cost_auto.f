...
c
      subroutine opts_cost_auto(ncol)
...
c
     ...
     .   "truncated to ", mlag," lags"
      nlag=min(mlag,nlag)
      call auto_auto(nlag,c0)
      end

c-------------------------------------------------------------------
c initial transformation on time series and its inverse
c
      subroutine cost_transform_auto(nmax,mcmax,nxdum,x)
      dimension x(nmax)
      parameter(mlag=100000)
      dimension c0(mlag), c(mlag)
      common /costcom/ nlag, c0, c, sd, sc, iweight
      ...
      subroutine cost_inverse_auto(nmax,mcmax,nxdum,x,y)
...
c
      subroutine cost_full_auto(iv)
      parameter(mlag=100000)
      dimension c0(mlag), c(mlag)
      common /costcom/ nlag, c0, c, sd, sc, iweight
      common nmax,cost

      call auto_auto(nlag,c)
      cc=0
      do 10 n=1,nlag
 10      call aver_auto(cc,c0(n)-c(n),n)
      cost=cc
      end
...
c
      subroutine cost_update_auto(nn1,nn2,cmax,iaccept,iv)
 ...
 1       call aver_auto(comp,c0(n)-cc,n)
         if(comp.ge.cmax) return
 10      ccop(n)=cc
      cost=comp  ! if got here: accept
      iaccept=1

c  notice that exch comes from the permutation routine
c  we use the fact that "auto" cost scheme is associated only with "random" permutation scheme
