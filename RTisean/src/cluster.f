...
c modified by Alexei Grigoriev, 27.4.2006 
	...
	subroutine cluster(options_filename)
	...
	call getparam(str,options_filename)	
      ...
      call whatido("clustering a dissimilarity matrix",iverb,str)
      ncl=imust("#",str)
      iflag=lopt("=",1,str)
      call stcan('X',filex,' ',str)
 ...
 998  if(nfix.eq.np) goto 997 
c 998  if(nfix.eq.np) stop "all fixed."
