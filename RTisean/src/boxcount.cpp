/* Author: Rainer Hegger Last modified: May 28, 2000 */
...
void ns_boxcount::show_options(char *progname)
{
...
void ns_boxcount::scan_options(int n,char **in)
{
  char *out;
...
hliste* ns_boxcount::make_histo(void)
{
  int i;
  hliste *element;
...
void ns_boxcount::next_dim(int wd,int n,unsigned int *first)
{
  int i,which,d1,comp;
  ...
  for (i=0;i<n;i++) {
    ...
    hf= ++found[which];
    check_alloc(act[which]= (unsigned int*)
...
void ns_boxcount::start_box(void)
{
  int i,which;
  double epsinv,norm,p;
  unsigned int **act;
...
int ns_boxcount::main(int argc,char **argv)
{

LENGTH=ULONG_MAX;exclude=0;
maxembed=10;dimension=1;DELAY=1;EPSCOUNT=20;
verbosity=0xff;
Q=2.0;EPSMIN=1.e-3;EPSMAX=1.0;
dimset=0;epsminset=0;epsmaxset=0;
outfile=NULL;
column=NULL;
  ...
  for (k=0;k<EPSCOUNT;k++) {
    ...
    start_box();
    histo_el=(hliste *)root;
    while (histo_el->ptr != NULL)
      histo_el=(hliste *)(histo_el->ptr);
