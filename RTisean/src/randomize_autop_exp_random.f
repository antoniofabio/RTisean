...
c modified by Alexei Grigoriev, 27.4.2006 
	subroutine randomize_autop_exp_random(options_filename)
...
c     data wr/0.9/, nsur/1/
	...
	call getparam(str,options_filename)
      ...
      call cost_transform_autop(nmax,mcmax,nx,xx)
 ...
 20            x(n,m)=xx(n,m)
         ...
         temp=cool_init_exp()
         call cost_init_autop()
         call permute_init_random()
         call cost_full_autop(iv_vcost(iverb))
         cmin=cost
         time=0
 1       time=time+1.
         call permute_random(n1,n2)
         cmax=cost-temp*log(rand(0.0))    ! maximal acceptable cost
         call cost_update_autop(n1,n2,cmax,iaccept,iv_vmatch(iverb))
     ...
     .         "after ",real(time)," steps at T=",temp," cost: ",cost
            cmin=cost
            call cost_inverse_autop(nmax,mcmax,nx,x,y)
