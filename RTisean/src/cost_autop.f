...
c
      subroutine opts_cost_autop(ncol)
...
c
     ...
     .   "truncated to ", mlag," lags"
      nlag=min(mlag,nlag)
      call auto_autop(nlag,c0)
      end

c-------------------------------------------------------------------
c initial transformation on time series and its inverse
c
      subroutine cost_transform_autop(nmax,mcmax,nxdum,x)
      dimension x(nmax)
      parameter(mlag=100000)
      dimension c0(mlag), c(mlag)
      common /costcom/ nlag, c0, c, sd, sc, iweight
      ...
      subroutine cost_inverse_autop(nmax,mcmax,nxdum,x,y)
...
c
      subroutine cost_full_autop(iv)
      parameter(mlag=100000)
      dimension c0(mlag), c(mlag)
      common /costcom/ nlag, c0, c, sd, sc, iweight
      common nmax,cost

      call auto_autop(nlag,c)
      cc=0
      do 10 n=1,nlag
 10      call aver_autop(cc,c0(n)-c(n),n)
      cost=cc
      end
...
c
      subroutine cost_update_autop(n1,n2,cmax,iaccept,iv)
      ...
      do 10 n=1,nlag
         ...
         call aver_autop(comp,c0(n)-cc,n)
         if(comp.ge.cmax) return
 10      ccop(n)=cc
      cost=comp  ! if got here: accept
      iaccept=1

c  "auto" cost scheme is only associated with "random" perm. scheme
...
c
      subroutine aver_autop(cav,dc,n)
      parameter(mlag=100000)
      dimension c0(mlag), c(mlag)
      common /costcom/ nlag, c0, c, sd, sc, iweight
      common nmax
