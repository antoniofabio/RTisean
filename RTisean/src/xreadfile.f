...
c read columns as seperate time series
 ...
 10      mlast=max(mlast,icol(i))
      if(mlast.gt.mline) goto 998
c      if(mlast.gt.mline) stop "xreadfile: make mline larger."
      lc=0
      do 20 n=1,nexcl
         lc=lc+1
...
c write comment and nmax points
      ...
      if(mmax.gt.1000) then
          goto 997
c         write(istderr(),*) "xwritecfile: "//
c     .      "cannot write more than 1000 columns"
c         stop
      endif
...
c write nmax points
      ...
      subroutine columns(mc,mmax,icol,str)
      dimension icol(*)

      call imcan("c",mmax,mc,icol,str)
      icmax=0
      do 10 m=1,mc
