...
void ns_nstep::show_options(char *progname)
{
...
void ns_nstep::scan_options(int n,char **in)
{
  char *out;
...
void ns_nstep::put_in_boxes(void)
{
  int i,j,n;
  static int hdim;
  double epsinv;
...
unsigned int ns_nstep::hfind_neighbors(void)
{
  char toolarge;
...
void ns_nstep::multiply_matrix(double **mat,double *vec)
{
  double *hvec;
  long i,j;
...
void ns_nstep::make_fit(int number,double *newcast)
{
...
void ns_nstep::make_zeroth(int number,double *newcast)
{
  unsigned long i,d;
  double *sj;
...
int ns_nstep::main(int argc,char **argv)
{

onscreen=1;epsset=0;outfile=NULL;
infile=NULL;
nmax=(NMAX-1);
verbosity=0xff;


embed=2;dim=1;DELAY=1;
column=NULL;dimset=0;do_zeroth=0;
MINN=30;
LENGTH=ULONG_MAX;FLENGTH=1000;exclude=0;
EPS0=1.e-3;EPSF=1.2;
