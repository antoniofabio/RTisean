...
c modified by Alexei Grigoriev, 27.4.2006 
	subroutine randomize_spikespec_exp_event(options_filename)
...
c     data wr/0.9/, nsur/1/
	...
	call getparam(str,options_filename)
      ...
      wr=fcan("u",wr)
      call opts_cost_spikespec(mcmax)
      call opts_cool_exp()
      call opts_permute_event()
      isout=igetout(fout,iverb)

      call nthstring(1,file)
      call xreadfile(nmax,mcmax,nx,xx,nexcl,icol,file,iverb)
      call cost_transform_spikespec(nmax,mcmax,nx,xx)
 ...
 20            x(n,m)=xx(n,m)
         ...
         temp=cool_init_exp()
         call cost_init_spikespec()
         call permute_init_event()
         call cost_full_spikespec(iv_vcost(iverb))
         cmin=cost
         time=0
 1       time=time+1.
         call permute_event(n1,n2)
         cmax=cost-temp*log(rand(0.0))    ! maximal acceptable cost
         call cost_update_spikespec(n1,n2,cmax,iaccept,iv_vmatch(iverb))
     ...
     .         "after ",real(time)," steps at T=",temp," cost: ",cost
            cmin=cost
            call cost_inverse_spikespec(nmax,mcmax,nx,x,y)
