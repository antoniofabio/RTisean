...
c      if(nmax.gt.nx) stop "d1: make nx larger."
      mt=(m-1)/mmax+1
      ncomp=nmax-(mt-1)*id
      ...
      if(k.eq.kpr) return
c   the following line should be uncommented and the output redirected to a temporary file
c     write(istderr(),*) 'Mass ', exp(pln),': k=', k, ', N=', ncomp 
