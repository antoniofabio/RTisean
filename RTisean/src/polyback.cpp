...
void ns_polyback::show_options(char *progname)
{
...
void ns_polyback::scan_options(int n,char **in)
{
  char *out;
...
double ns_polyback::polynom(unsigned long act,unsigned int which)
{
  unsigned int i,j;
  double ret=1.0,h;
...
void ns_polyback::make_fit(void)
{
  double **mat,*vec;
  double h;
  unsigned long n;
...
double ns_polyback::forecast_error(unsigned long i0,unsigned long i1)
{
  unsigned int i;
  unsigned long n;
  double h,error=0.0;
...
int ns_polyback::main(int argc,char **argv)
{
...
plength=UINT_MAX;
column=1;dim=2;delay=1;down_to=1;step=1;
verbosity=0xff;
