...
void ns_rbf::show_options(char *progname)
{
...
void ns_rbf::scan_options(int n,char **in)
{
  char *out;
...
double ns_rbf::rbf(double *act,double *cen)
{
  static double denum;
  double r=0;
  int i;
...
double ns_rbf::forecast_error(unsigned long i0,unsigned long i1)
{
  int i,n;
  double h,error=0.0;
...
void ns_rbf::make_cast(FILE *out)
{
  double *cast,ynew;
  int i,n,dim;
  ...
  for (n=0;n<CLENGTH;n++) {
    ynew=coefs[0];
    for (i=1;i<=CENTER;i++)
      ynew += coefs[i]*rbf(&cast[dim],center[i-1]);
    fprintf(out,"%e\n",ynew*interval+min);
    for (i=0;i<dim;i++)
      cast[i]=cast[i+1];
    cast[dim]=ynew;
...
int ns_rbf::main(int argc,char **argv)
{

outfile=NULL;stdo=1;MAKECAST=0;
infile=NULL;
setdrift=1;
DIM=2;DELAY=1;CENTER=10;STEP=1;
COLUMN=1;
verbosity=0xff;
CLENGTH=1000;
LENGTH=ULONG_MAX;INSAMPLE=ULONG_MAX;exclude=0;
