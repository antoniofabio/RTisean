...
void ns_d2::show_options(char *progname)
{
...
void ns_d2::scan_options(int n,char **argv)
{
  char *out;
...
int ns_d2::main(int argc,char **argv)
{

dimset=0;rescale_set=0;eps_min_set=0;eps_max_set=0;
FOUT=NULL;
EPSMAX=1.0;EPSMIN=1.e-3;
imax=NMAX-1;
MINDIST=0;MAXFOUND=1000;
length=ULONG_MAX;exclude=0;
DIM=1;EMBED=10;HOWOFTEN=100;DELAY=1;
verbosity=0x1;
column=NULL;
infile=NULL;
  ...
  if (FOUT == NULL) {
    if (!stdi) {
      check_alloc(FOUT=(char *)calloc(strlen(infile)+1,(size_t)1));
      strcpy(FOUT,infile);
    }
    else {
      check_alloc(FOUT=(char *)calloc((size_t)6,(size_t)1));
      strcpy(FOUT,"stdin");
    }
  }
  if (column == NULL)
